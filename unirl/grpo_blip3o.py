...
from .trainer import T2IGRPOTrainer, T2ICoTGRPOTrainer, I2IGRPOTrainer
...
def main(script_args, training_args, model_args):
    ...
    image_holder = f"<|vision_start|>{UND_IMAGE_TOKEN}<vision_end>\n"
    if script_args.task == "t2icot":
        QUESTION_TEMPLATE = "Please enhance the following prompt to make it more suitable for image generation: {Question}. Output the final answer (entailed prompt) in <answer> </answer> tags."
    elif script_args.task == "i2i":
        QUESTION_TEMPLATE = image_holder + "Please edit the image given the following instruction: {Question}"
    else:  # t2i
        QUESTION_TEMPLATE = "Please generate the image given the following caption: {Question}"
    ...
    trainer_cls = {
        "t2i": T2IGRPOTrainer,
        "t2icot": T2ICoTGRPOTrainer,
        "i2i": I2IGRPOTrainer
    }.get(script_args.task, T2IGRPOTrainer)
