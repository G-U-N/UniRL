...
# Launch GenEval prompt-refinement GRPO training with the FLUX.2-klein-base T2I
...
RUN_NAME="${RUN_NAME:-qwenklein-geneval-${TRAINER_TAG}-grpo}"
...
# Klein-base T2I defaults: 20 inference steps, first 8 with SDE log-prob tracking.
# Forwarded via env in case downstream code reads them (the trainer also reads
# --num_inference_steps directly; these env vars are the override surface).
export PROMPTRL_GENEVAL_GUIDANCE_SCALE="${PROMPTRL_GENEVAL_GUIDANCE_SCALE:-${GUIDANCE_SCALE:-4.0}}"
