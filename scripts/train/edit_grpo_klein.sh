...
# Launch joint LLM+DiT GRPO training with the FLUX.2-klein-base diffusion expert.
# Mirrors edit_grpo.sh; differs in the default RUN_NAME and OUTPUT_DIR and in
# downstream defaults appropriate for the klein backbone (Qwen3 text encoder,
# no CLIP). The trainer auto-detects the backbone from MODEL_NAME_OR_PATH
# (must contain the substring "qwenklein").

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$REPO_DIR"

: "${MODEL_NAME_OR_PATH:?Set MODEL_NAME_OR_PATH to a pretrained Qwen-Klein checkpoint (the path must contain 'qwenklein').}"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-2,3,4,5,6,7}"
NPROC_PER_NODE="${NPROC_PER_NODE:-6}"
MASTER_ADDR="${MASTER_ADDR:-localhost}"
MASTER_PORT="${MASTER_PORT:-25000}"
RUN_NAME="${RUN_NAME:-qwenklein-edit-grpo}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/rl/klein/$RUN_NAME}"
...
# Klein-base defaults: guidance 4.0 (Klein's documented default vs Kontext's 2.5),
# 8 sampling steps per the user's preference (matching Kontext for direct comparison).
# Klein-base is undistilled, so 8 steps will sample below the 50-step quality ceiling;
# bump PROMPTRL_EDIT_NUM_INFERENCE_STEPS upward if reward signal looks noisy.
export PROMPTRL_EDIT_GUIDANCE_SCALE="${PROMPTRL_EDIT_GUIDANCE_SCALE:-${EDIT_GUIDANCE_SCALE:-3.5}}"
