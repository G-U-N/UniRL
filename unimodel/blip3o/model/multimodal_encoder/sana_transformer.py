...
from diffusers.models.attention_processor import AttentionProcessor, Attention, SanaLinearAttnProcessor2_0
...
class SanaTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.ref_linear = nn.Linear(inner_dim, inner_dim)
        nn.init.zeros_(self.ref_linear.weight)
        nn.init.zeros_(self.ref_linear.bias)
    ...
    def forward(
        ...
        if ref_hidden_states is None:
            ref_hidden_states = torch.zeros_like(hidden_states)
            
        hidden_states = self.patch_embed(hidden_states) + self.ref_linear(self.ref_patch_embed(ref_hidden_states))
